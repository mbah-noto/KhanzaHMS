...
public final class DlgPenyakit extends javax.swing.JDialog {
    ...
    public DlgPenyakit(java.awt.Frame parent, boolean modal) {
        ...
        TNm.setDocument(new batasInput((byte)35).getKata(TNm));
        ...
        ktg.getTable().addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                kd_ktg.setText(ktg.getTextField().getText());    
                Sequel.cariIsi("select kategori_penyakit.nm_kategori from kategori_penyakit where kategori_penyakit.kd_ktg='"+kd_ktg.getText()+"'", TKtg);
            }
        ...
        isForm(); 
        try{
            ps=koneksi.prepareStatement("select kd_penyakit,nm_penyakit,ciri_ciri,keterangan, "+
            ...
            ps2=koneksi.prepareStatement("select count(kd_penyakit) as jumlah from kategori_penyakit inner join penyakit "+
                " on penyakit.kd_ktg=kategori_penyakit.kd_ktg where  "+
                " kd_penyakit like ? or "+
                " nm_penyakit like ? or "+
                " ciri_ciri like ? or "+
    ...
    private void initComponents() {
        ...
        TKeterangan.setBounds(103, 89, 516, 23);
        ...
        BtnSeek.setBounds(591, 120, 28, 23);
        ...
        ChkInput.setBackground(new java.awt.Color(240, 245, 235));
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            Sequel.menyimpan("penyakit","'"+TKd.getText()+"','"+
                    TNm.getText()+"','"+
                    TCiri.getText()+"','"+
                    TKeterangan.getText()+"','"+
                    kd_ktg.getText()+"'","Kode Penyakit");
            tampil();
            emptTeks();
        }
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        Valid.hapusTable(tabMode,TKd,"penyakit","kd_penyakit");
        tampil();
        emptTeks();
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            Valid.editTable(tabMode,"penyakit","kd_penyakit",TKd,"nm_penyakit='"+TNm.getText()+
                    "',ciri_ciri='"+TCiri.getText()+
                    "',keterangan='"+TKeterangan.getText()+
                    "',kd_ktg='"+kd_ktg.getText()+"'");
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            Valid.MyReport("rptPenyakit.jrxml","report","::[ Data Penyakit ]::","select kd_penyakit,nm_penyakit,ciri_ciri,keterangan, "+
                "nm_kategori,ciri_umum "+
                "from kategori_penyakit inner join penyakit "+
                "on penyakit.kd_ktg=kategori_penyakit.kd_ktg where  "+
                " kd_penyakit like '%"+TCari.getText().trim()+"%' or "+
                " nm_penyakit like '%"+TCari.getText().trim()+"%' or "+
                " ciri_ciri like '%"+TCari.getText().trim()+"%' or "+
                " keterangan like '%"+TCari.getText().trim()+"%' or "+
                " nm_kategori like '%"+TCari.getText().trim()+"%' or "+
                " ciri_umum like '%"+TCari.getText().trim()+"%' "+
                "order by kd_penyakit LIMIT "+awal+",500");
        }
...
private void kd_ktgKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kd_ktgKeyPressed
       if(evt.getKeyCode()==KeyEvent.VK_ENTER){
            Sequel.cariIsi("select kategori_penyakit.nm_kategori from kategori_penyakit where kategori_penyakit.kd_ktg='"+kd_ktg.getText()+"'", TKtg);// TODO add your handling code here:
        }else{
            Valid.pindah(evt, TKd,BtnSimpan);
        }
  ...
  isForm();                
    ...
    */
    ...
    public void emptTeks() {
        ...
        kd_ktg.setText("");
        TKtg.setText("");
        Valid.autoNomer("penyakit","P",9,TKd);
        TKd.requestFocus();
    }
    ...
    private void getData() {
        ...
        if(row!= -1){
            TKd.setText(tbPenyakit.getValueAt(row,0).toString());
            TNm.setText(tbPenyakit.getValueAt(row,1).toString());
            TCiri.setText(tbPenyakit.getValueAt(row,2).toString());
            TKeterangan.setText(tbPenyakit.getValueAt(row,3).toString());
            TKtg.setText(tbPenyakit.getValueAt(row,4).toString());
            Sequel.cariIsi("select kd_ktg from penyakit where kd_penyakit=?", kd_ktg,tbPenyakit.getValueAt(row,0).toString());
        }
    }
