...
public class DlgCariPiutang extends javax.swing.JDialog {
    ...
    public DlgCariPiutang(java.awt.Frame parent, boolean modal) {
        ...
        BtnPrint.setEnabled(var.getpiutang());     
    }
    ...
    private void initComponents() {
        ...
        ppUbah.setBackground(new java.awt.Color(242, 242, 242));
        ppUbah.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        ppUbah.setForeground(new java.awt.Color(102, 51, 0));
        ppUbah.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/inventaris.png"))); // NOI18N
...
*/
    ...
    private void ppCetakNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppCetakNotaActionPerformed
        ...
        if(Kd2.getText().trim().equals("")){
            ...
            if(pilih>0){
                if(pilih==1){
                    Valid.panggilUrl("nota/index7.php?nonota="+Kd2.getText());
                }else if(pilih==2){
                    Valid.panggilUrl("nota/index8.php?nonota="+Kd2.getText());
                }else if(pilih==3){
                    Valid.panggilUrl("nota/index15.php?nonota="+Kd2.getText());
                }else if(pilih==4){
                    Valid.panggilUrl("nota/index16.php?nonota="+Kd2.getText());
                }
            }
        }
        this.setCursor(Cursor.getDefaultCursor());
  ...
  if(Kd2.getText().trim().equals("")){
     ...
     try {
         PreparedStatement pscaripiutang=koneksi.prepareStatement(
                 "select nota_piutang, kd_bangsal from piutang where nota_piutang='"+Kd2.getText()+"'");
         ...
         while(rscaripiutang.next()){
             ...
             Sequel.menyimpan("tampjurnal","'71000','PIUTANG PASIEN','0','"+Sequel.cariIsi("select sum(total) from detailpiutang where nota_piutang='"+rscaripiutang.getString("nota_piutang")+"'")+"'","Rekening");    
             Sequel.menyimpan("tampjurnal","'11120','KAS DI TANGAN','"+Sequel.cariIsi("select sum(total) from detailpiutang where nota_piutang='"+rscaripiutang.getString("nota_piutang")+"'")+"','0'","Rekening"); 
            jur.simpanJurnal(rscaripiutang.getString("nota_piutang"),Sequel.cariIsi("select current_date()"),"U","BATAL PIUTANG DI "+Sequel.cariIsi("select nm_bangsal from bangsal where kd_bangsal='"+rscaripiutang.getString("kd_bangsal")+"'").toUpperCase());
         }          
         Sequel.queryu("delete from piutang where nota_piutang='"+Kd2.getText()+"'");
         tampil();
    ...
    */
    ...
    public void tampil() {
        String tanggal="  piutang.tgl_piutang between '"+Tgl1.getSelectedItem()+"' and '"+Tgl2.getSelectedItem()+"' ";
        ...
        String sql="select piutang.nota_piutang, piutang.tgl_piutang, "+
                    ...
                    " from piutang inner join petugas inner join bangsal   "+
        ...
        try{
            ...
            while(rs.next()){
                ...
                ResultSet rs2=stat2.executeQuery("select detailpiutang.kode_brng,databarang.nama_brng, detailpiutang.kode_sat,"+
                        ...
                        " detailpiutang inner join databarang inner join kodesatuan "+
                        " on detailpiutang.kode_brng=databarang.kode_brng "+
                        ...
                        " detailpiutang.nota_piutang='"+rs.getString(1)+"' "+sat+bar+" and kodesatuan.satuan like '%"+TCari.getText()+"%' order by detailpiutang.kode_brng  ");
                subttlall=0;
                subttldisc=0;
                subttljual=0;
                sisapiutang=0;
                ...
                sisapiutang=rs.getDouble(11)-Sequel.cariIsiAngka("select sum(besar_cicilan) from bayar_piutang where no_rkm_medis='"+rs.getString(5)+"' ");
                cicilan=Sequel.cariIsiAngka("select sum(besar_cicilan) from bayar_piutang where no_rkm_medis='"+rs.getString(5)+"' ");
                if(sisapiutang<1){
                    status="Lunas";
