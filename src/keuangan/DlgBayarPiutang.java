...
public final class DlgBayarPiutang extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Cicilan.setBounds(650, 10, 120, 23);

        label16.setText("Pasien :");
        label16.setName("label16"); // NOI18N
        label16.setPreferredSize(new java.awt.Dimension(60, 23));
        panelisi4.add(label16);
        label16.setBounds(187, 10, 80, 23);
        ...
        BtnSeek.setBounds(524, 10, 28, 23);
        ...
        if(NoRawat.getText().trim().equals("")){
            ...
            try {
                    koneksi.createStatement().execute(
                        "insert into bayar_piutang values('"+Tanggal.getSelectedItem()+"','"+Kdmem.getText()+"','"+Cicilan.getText()+
                            "','"+Keterangan.getText()+"','"+NoRawat.getText()+"')");  
                    
                    if(Sisa.getText().equals("0")){
                        Sequel.mengedit("piutang_pasien","no_rawat='"+NoRawat.getText()+"'","status='Lunas'");
                    }
                    
                    ResultSet rs2=koneksi.createStatement().executeQuery("select * from rekening where kd_rek='72000'");
                    if(!rs2.next()){
                    ...
                    ResultSet rs4=koneksi.createStatement().executeQuery("select * from rekeningtahun where thn='"+Tanggal.getSelectedItem().toString().substring(0,4) +"' and kd_rek='72000'");
                    if(!rs4.next()){
                        koneksi.createStatement().execute("insert into rekeningtahun values('"+Tanggal.getSelectedItem().toString().substring(0,4) +"','72000','0')");
                    }
                    
                    ResultSet rs5=koneksi.createStatement().executeQuery("select * from rekeningtahun where thn='"+Tanggal.getSelectedItem().toString().substring(0,4) +"' and kd_rek='11120'");
                    if(!rs5.next()){
                        koneksi.createStatement().execute("insert into rekeningtahun values('"+Tanggal.getSelectedItem().toString().substring(0,4) +"','11120','0')");
                    }
                    
                    Sequel.queryu("delete from tampjurnal");                    
                    Sequel.menyimpan("tampjurnal","'72000','BAYAR PIUTANG','0','"+Cicilan.getText()+"'","Rekening");    
                    Sequel.menyimpan("tampjurnal","'11120','KAS DI TANGAN','"+Cicilan.getText()+"','0'","Rekening"); 
                    jur.simpanJurnal(NoRawat.getText(),Tanggal.getSelectedItem().toString(),"U","BAYAR PIUTANG");
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        Valid.hapusTable(tabMode,Kdmem,"bayar_piutang","tgl_bayar='"+Tanggal.getSelectedItem()+"' and no_rkm_medis");
        Sequel.queryu("delete from jurnal where no_bukti='"+NoRawat.getText()+"' "+
                     " and tgl_jurnal='"+Tanggal.getSelectedItem()+"' "+
                     " and jenis='U' and keterangan='BAYAR PIUTANG'");        
        BtnCariActionPerformed(evt);
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            Valid.MyReport("rptBayar.jrxml","report","::[ Bayar Piutang ]::","select bayar_piutang.tgl_bayar, bayar_piutang.no_rkm_medis,pasien.nm_pasien, bayar_piutang.besar_cicilan,"+
                ...
                "bayar_piutang.tgl_bayar like '%"+TCari.getText()+"%' order by bayar_piutang.tgl_bayar,bayar_piutang.no_rkm_medis,bayar_piutang.no_rawat ");
        }
...
private void ppNotaPiutangBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppNotaPiutangBtnPrintActionPerformed
    this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        if(tabMode.getRowCount()==0){
            ...
            //System.out.println(tabMode.getValueAt(tbKamar.getSelectedRow(),0).toString());
            //System.out.println(tabMode.getValueAt(tbKamar.getSelectedRow(),1).toString());
            //System.out.println(tabMode.getValueAt(tbKamar.getSelectedRow(),2).toString());
            //System.out.println(tabMode.getValueAt(tbKamar.getSelectedRow(),3).toString());
            //System.out.println(tabMode.getValueAt(tbKamar.getSelectedRow(),4).toString());
            //System.out.println(tabMode.getValueAt(tbKamar.getSelectedRow(),5).toString());
...
private void BtnSeekActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSeekActionPerformed
    ...
    */
    ...
    public void isCek(){
        BtnSimpan.setEnabled(var.getbayar());
        BtnHapus.setEnabled(var.getbayar());
        BtnPrint.setEnabled(var.getbayar());
    }
}
