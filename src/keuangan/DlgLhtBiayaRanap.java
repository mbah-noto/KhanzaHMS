 ...
 * @author perpustakaan
 */
public final class DlgLhtBiayaRanap extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgLhtBiayaRanap(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Data Tagihan Ranap Masuk ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 70, 40))); // NOI18N
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgLhtBiayaRanap dialog = new DlgLhtBiayaRanap(new javax.swing.JFrame(), true);
    ...
    public void tampil(){
        String sql="select rawat_jl_dr.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien "+
                "from rawat_jl_dr inner join reg_periksa inner join pasien "+
                "on rawat_jl_dr.no_rawat=reg_periksa.no_rawat "+
                "and reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                "where tgl_registrasi between '"+Tgl1.getSelectedItem()+"' and '"+
                Tgl2.getSelectedItem()+"' group by reg_periksa.no_rawat ";
        prosesCari(sql);
    }
    ...
    private void prosesCari(String sql) {
        ...
        try{
            ResultSet rs=koneksiDB.condb().createStatement().executeQuery(sql);
            all=0;
            while(rs.next()){
                Object[] data={rs.getString("no_rkm_medis"),
                                    rs.getString("nm_pasien"),
                                    Valid.SetAngka(Sequel.cariIsiAngka("select sum(billing.totalbiaya) from billing where status='Registrasi' "+
                                    "and no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat")),
                                    Valid.SetAngka(Sequel.cariIsiAngka("select sum(jns_perawatan.total_byrdr) from jns_perawatan inner join rawat_inap_dr "+
                                    "on rawat_inap_dr.kd_jenis_prw=jns_perawatan.kd_jenis_prw where no_rawat='"+rs.getString("no_rawat")+"'")),
                                    Valid.SetAngka(Sequel.cariIsiAngka("select sum(billing.totalbiaya) from billing where status='Obat' "+
                                    "and no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat")),
                                    Valid.SetAngka(Sequel.cariIsiAngka("select sum(billing.totalbiaya) from billing where status='Ralan Dokter' "+
                                    "and no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat")+
                                    Sequel.cariIsiAngka("select sum(billing.totalbiaya) from billing where status='Ralan Paramedis' "+
                                    "and no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat")),
                                    "",
                                    Valid.SetAngka(Sequel.cariIsiAngka("select sum(billing.totalbiaya) from billing where no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat")),
                                    Sequel.cariIsi("select nm_perawatan from billing where no_rawat='"+rs.getString("no_rawat")+"' and status='Dokter' "),
                                    Sequel.cariIsi("select if(sum(billing.totalbiaya)>0,'Lunas','Belum Lunas') from billing where no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat")};
                tabMode.addRow(data);
                all=all+Sequel.cariIsiAngka("select sum(billing.totalbiaya) from billing where no_rawat='"+rs.getString("no_rawat")+"' group by billing.no_rawat");
            }
